...
//                startActivity(intent);
                new SmartRun() {
                    ...
                    @UIThread
                    public void action() {
                        ViewUtils.introduceView(v, "这个是展示的控件  " + ThreadUtils.getThreadStytle().toString(), true, new
                    ...
                    public void dismiss() {
                        if (prepare()) return;
                        ViewUtils.introduceView(tv2, "这个是展示的控件2  " + ThreadUtils.getThreadStytle().toString(), true,
...
//                        mLog.i("dismiss(String a)");
                        tv2.setText("即将调用测试");
                        ViewUtils.introduceView(tv2, a + ThreadUtils.getThreadStytle().toString(), true, new
                                PopupWindow.OnDismissListener() {
                                    @Override
                                    public void onDismiss() {
                                        test();
...
//                        test();
                    ...
                    public void test() {
                        if (prepare()) return;
                        ViewUtils.introduceView(v, "测试下异步方法  " + ThreadUtils.getThreadStytle().toString(), true, new
                                PopupWindow.OnDismissListener() {
                            @Override
                            public void onDismiss() {
                                dismiss();
